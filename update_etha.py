#!/usr/bin/env python3
"""
Fetches ETHA Daily Holdings from BlackRock and pushes
numerator (quantity held) and denominator (shares outstanding) to the TOB API.

Usage:
    API_KEY=yourkey venv/bin/python update_etha.py
...
async def main():
    try:
        ...
        if "NOT FOUND" in result.values():
            raise RuntimeError("Could not parse all fields from XLS")
